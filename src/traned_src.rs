...
#[derive(Default)]
pub struct TranedSrc {
  path: PathBuf,
  // path to_lang from_lang src_hash , 用 BTreeMap 保证每次保存的yml顺序是一样的
  pub map: BTreeMap<String, BTreeMap<u32, (u32, Vec<u8>)>>,
  pub has_change: bool,
}

impl TranedSrc {
  pub fn get(&self, path: impl AsRef<str>, from_lang: u32, to_lang: u32) -> Option<Vec<u8>> {
    if let Some(m) = self.map.get(path.as_ref())
  ...
  pub fn add(&mut self, path: impl Into<String>, from_lang: u32, from_hash: Vec<u8>, to_lang: u32) {
  ...
  pub fn save(&self) -> Void {
    ...
    // path from_lang from_hash to_lang_li
    for (path, map) in &self.map {
      if let Yaml::Mapping(h) = from_rel_hash_to_lang_li
        .entry(Yaml::value_from_cow(path.into()))
...
pub fn load(path: impl Into<PathBuf>) -> Result<TranedSrc> {
  let path = path.into();
  let mut map = BTreeMap::new();
  if path.exists() {
    for i in Yaml::load_from_str(&ifs::rstr(&path)?)? {
      if let Yaml::Mapping(i) = i {
        for (path, h) in i {
            ...
            && let Yaml::Mapping(h) = h
          {
            ...
            let map = map.entry(path).or_insert_with(BTreeMap::new);
